...
:heavy_check_mark: **[vars_speed]** : List first, second component and variable name. To calcul variable speed with two components and export the file with new abbreviation variable. First (or n) element of "compo1" vector correspond to first (or n) element of "compo2" vector. If one short names of variable is same to cmip6 and copernicus data, write one time. WARNING !! : If two different variables have same component name, bug in the code. This target will be modified by speedCompo_cmip function to integrate depth variables created during process (chl50-100, uo0-100, etc...). For example, final name of the variable named "wind" in this target and calculated with "uo" and "vo" components will be "uovo" (two component names will be pasted)
