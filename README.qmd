...
:heavy_check_mark: **[match_name]** : List of two vectors. Allow to correspondance between "copernicus" and "cmip" variables during calcul of delta in change factor method. List uniquelly variables you want bias-corrected. These vectors must to contain all variables name create during process (variable processed by speed_vars and deep_level targets). The last vector "copy" contain copernicus variable names with fixe temporal resolution and they don't have correspondance with cmip6 but you want copy and rename into the output folder ("output/data_cmip6_change_factor/variables_bias-corrected"). Given variable names after the rename step with renameVar() function and **renameVar** target.
...
:arrow_forward: To download Copernicus data it's necessary to install [python3](https://www.python.org/downloads/) and motuclient (Ubuntu command: *python3 -m pip install motuclient==1.8.4 --no-cache-dir*).
