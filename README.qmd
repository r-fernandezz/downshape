...
:bulb: You can run only one part of *make.R* script when you commit targets of the part not used.
...
:heavy_check_mark: **[historical_period]** : List start and end time. To define min and max time of cmip6 data historical period (cdo format: "YYYY-MM-DDThh:mm:ss"). It must existe a time overlap with *time_span* target.

:heavy_check_mark: **[future_period]** : List start and end time. To define min and max time of cmip6 data future period (cdo format: "YYYY-MM-DDThh:mm:ss"). It must existe a time overlap with *time_span* target.
